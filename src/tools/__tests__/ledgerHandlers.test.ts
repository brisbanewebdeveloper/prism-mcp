...
function makeStorageStub() {
  return {
    saveLedger: vi.fn<StorageBackend["saveLedger"]>(() => Promise.resolve([{ id: "entry-uuid-001", created_at: new Date().toISOString() }])),
    patchLedger: vi.fn(() => Promise.resolve()),
    getLedgerEntries: vi.fn<StorageBackend["getLedgerEntries"]>(() => Promise.resolve([])),
    deleteLedger: vi.fn(() => Promise.resolve([])),
    softDeleteLedger: vi.fn(() => Promise.resolve()),
    hardDeleteLedger: vi.fn(() => Promise.resolve()),
    saveHandoff: vi.fn<StorageBackend["saveHandoff"]>(() => Promise.resolve({ status: "created", version: 1 })),
    getHandoffAtVersion: vi.fn<StorageBackend["getHandoffAtVersion"]>(() => Promise.resolve(null)),
    deleteHandoff: vi.fn(() => Promise.resolve()),
    loadContext: vi.fn<StorageBackend["loadContext"]>(() => Promise.resolve(null)),
    searchKnowledge: vi.fn(() => Promise.resolve(null)),
    searchMemory: vi.fn(() => Promise.resolve([])),
    saveHistorySnapshot: vi.fn<StorageBackend["saveHistorySnapshot"]>(() => Promise.resolve()),
    getHistory: vi.fn<StorageBackend["getHistory"]>(() => Promise.resolve([])),
    listProjects: vi.fn<StorageBackend["listProjects"]>(() => Promise.resolve([])),
...
function getFirstCallArg<T>(mockFn: { mock: { calls: unknown[][] } }): T {
...
describe("ledgerHandlers", () => {
  ...
  describe("sessionSaveLedgerHandler", () => {
    ...
    it("passes sanitized summary to storage", async () => {
      ...
      const callArg = getFirstCallArg<LedgerEntry>(storage.saveLedger);
  ...
  describe("sessionSaveHandoffHandler", () => {
    ...
    it("passes sanitized summary to storage", async () => {
      ...
      const callArg = getFirstCallArg<HandoffEntry>(storage.saveHandoff);
  ...
  describe("sessionSaveImageHandler", () => {
    ...
    it("updates handoff metadata with visual memory entry", async () => {
      ...
      const metadata = callArg.metadata as {
        visual_memory?: Array<{ description?: string }>;
      } | undefined;
      expect(metadata?.visual_memory).toHaveLength(1);
      expect(metadata?.visual_memory?.[0]?.description).toBe("UI mockup");
    });
