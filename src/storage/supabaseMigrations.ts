...
export const MIGRATIONS: Migration[] = [
  ...
  {
    version: 42,
    name: "fix_prism_create_link_ambiguity",
    sql: `
      -- v7.8.3: Avoid PL/pgSQL ambiguity between RETURNS TABLE output vars and memory_links columns
      CREATE OR REPLACE FUNCTION public.prism_create_link(
        p_user_id TEXT,
        p_source_id UUID,
        p_target_id UUID,
  ...
  {
    version: 43,
    name: "embedding_retry_state",
    sql: `
      -- v7.8.4: Persistent retry metadata for ledger embedding repair
  ...
  {
    version: 45,
    name: "role_scoped_semantic_search_handoff_embeddings",
    sql: `
      -- Keep Supabase handoff embeddings in parity with the storage layer.
      ...
      -- The storage layer supplies p_role to keep multi-agent searches scoped.
