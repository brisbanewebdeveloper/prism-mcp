...
const CODEX_STARTUP_BODY = [
  ...
  "prompt-keyword skill routing fire on turn one; pass `{}` only when there is no user message. Emit no",
...
function serializeClaudeStartupBlock(newline: string): string {
  return [
    ...
    "`{prompt: \"<first user message>\"}` so prompt-keyword skill routing fires on turn one. Pass `{}` only when",
