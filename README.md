...
**That's it.** Restart your client. All tools are available. The **Mind Palace Dashboard** (the visual UI for your agent's brain) starts automatically at `http://localhost:3000` for direct `npx` usage. You don't need to keep a tab open — the dashboard runs in the background and the MCP tools work with or without it.
...
Copy `.env.example` to `.env`, then adjust `PRISM_DB_HOST_PORT`, `PRISM_REST_HOST_PORT`, `PRISM_DASHBOARD_PORT`, and any optional API keys you want to expose inside the Prism container. Running `docker compose up -d` starts `db`, `rest`, and a `prism` app container that serves the dashboard on `PRISM_DASHBOARD_PORT`.

Keep `PRISM_SUPABASE_URL=http://rest:3000` and `PRISM_SUPABASE_API_PREFIX=` because the checked-in stack talks to raw PostgREST at the root instead of `/rest/v1`. If `PRISM_SUPABASE_KEY` is unset, `docker-start-prism.sh` auto-generates a local JWT signed by `PRISM_REST_JWT_SECRET` and uses `PRISM_DB_USER` as the role claim, so the Prism container can authenticate against the same local PostgREST role used by the stack. The Compose stack also persists Prism's local `~/.prism-mcp` state in the `prism_state` volume, so dashboard settings survive `docker compose down`; use `docker compose down -v` when you intentionally want a full reset.

### Streamable HTTP Transport

For HTTP-native MCP clients, set `PRISM_MCP_TRANSPORT=http`, then connect to `http://localhost:<PRISM_MCP_PORT><PRISM_MCP_PATH>` when running Prism directly or `http://localhost:<PRISM_MCP_HOST_PORT><PRISM_MCP_PATH>` when going through the Docker stack. The default transport remains stdio.
...
> 🔑 The core Mind Palace works **100% offline** with zero API keys. Cloud keys unlock semantic search, web search, research, and automation features. See [Environment Variables](#environment-variables).

> **Optional API keys:** `GOOGLE_API_KEY` for semantic search, Briefings, and compaction; `GOOGLE_SEARCH_CREDENTIALS` (or `GOOGLE_SEARCH_API_KEY` + `GOOGLE_SEARCH_CX`) for web search; `BRAVE_API_KEY` for Brave local search and Scholar discovery; `FIRECRAWL_API_KEY` or `TAVILY_API_KEY` for Scholar scraping; and `BRAVE_ANSWERS_API_KEY` or `PRISM_BRAVE_ANSWERS_API_KEY` for `brave_answers`.
...
> ```
> GOOGLE_API_KEY=...                                 # Semantic search, Briefings, compaction, Gemini tools
...
| `PRISM_DISABLE_BRAVE_WEB_SEARCH_CODE_MODE` | No | `"true"` to hide `brave_web_search_code_mode` from normal runtime discovery and degrade direct calls to standard `brave_web_search` (Google Programmable Search) |
